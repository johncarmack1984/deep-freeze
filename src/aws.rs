...
pub async fn upload_to_s3(
    ...
    //     local_path, s3_bucket, s3_path
    // ))
    // .with_default(true)
    // .prompt()
    // {
    //     Ok(true) => println!("🚀  Starting Upload"),
    //     Ok(false) => {
    ...
    match localfs::get_local_size(&local_path) {
        // size if size < (MIN_CHUNK_SIZE * 10).try_into().unwrap() => {
        size if size > 0 => singlepart_upload(&aws_client, &s3_path, &local_path, &s3_bucket).await,
        _ => panic!("file has no size"), // _ => multipart_upload(&aws_client, &s3_path, &local_path, &s3_bucket).await,
    }
}
